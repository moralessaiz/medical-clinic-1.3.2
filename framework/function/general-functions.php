...
function medical_clinic_register_js_scripts() {
	...
	$cmsmasters_localize_array = array( 
		...
		'gmap_api_key' => 						$cmsmasters_option['medical-clinic' . '_gmap_api_key'], 
		'gmap_api_key_notice' => 				esc_html__('Please add your Google Maps API key', 'medical-clinic'), 
		'gmap_api_key_notice_link' => 			esc_html__('read more how', 'medical-clinic'), 
	...
	wp_enqueue_script('medical-clinic-theme-script', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/js/jquery.theme-script.js', array('jquery', 'medical-clinic-script'), '1.0.0', true);
	...
	if ( 
		...
		isset($cmsmasters_option['medical-clinic' . '_gmap_api_key']) && 
		$cmsmasters_option['medical-clinic' . '_gmap_api_key'] != '' 
	) {
...
if (!function_exists('medical_clinic_theme_fonts_generate')) {

function medical_clinic_theme_fonts_generate() {
	$cmsmasters_option = medical_clinic_get_global_options();
	...
	foreach ($font_fields as $font_field) {
		$font_option = 'medical-clinic' . '_' . $font_field . '_font_google_font';
...
add_action('wp_enqueue_scripts', 'medical_clinic_theme_fonts_generate');
add_action('enqueue_block_editor_assets', 'medical_clinic_theme_fonts_generate');
...
function cmsmasters_theme_font($fonts, $font_name = '') {
	...
	if ( 
		...
		if ($web_fonts != '') {
			...
			if (
				isset($cmsmasters_option['medical-clinic' . '_google_web_fonts_subset']) && 
				is_array($cmsmasters_option['medical-clinic' . '_google_web_fonts_subset'])
			) {
				$web_fonts_subset_array = $cmsmasters_option['medical-clinic' . '_google_web_fonts_subset'];
			} else {
...
function medical_clinic_the_widgets_init() {
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_default', 
            'description' => esc_html__('Widgets in this area will be shown in all left and right sidebars till you don\'t use custom sidebar. To display the sidebar please choose the Right or Left Layout Type in the Theme Settings - General - Content options.', 'medical-clinic'), 
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_bottom', 
            'description' => esc_html__('Widgets in this area will be shown at the bottom of middle block below the content and middle sidebar, but above footer. To display Bottom Sidebar please enable it in the Theme Settings - General - Content - Bottom Sidebar Visibility by Default. You can also display it on concrete page in the Page Layout options.', 'medical-clinic'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_archive', 
            'description' => esc_html__('Widgets in this area will be shown in all left and right sidebars on archives pages. To display the sidebar please choose the Right or Left Layout Type in the Theme Settings - General - Content options.', 'medical-clinic'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_search', 
            'description' => esc_html__('Widgets in this area will be shown in left or right sidebar on search page. To display the sidebar please choose the Right or Left Layout Type in the Theme Settings - General - Content options.', 'medical-clinic'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
...
function medical_clinic_excerpt($length = 55, $show = true) {
	if ($show) {
...
function medical_clinic_generate_front_css() {
	$cmsmasters_page_id = medical_clinic_get_singular_id();
...
function cmsmasters_get_tweets( $username, $count ) {
	$backup_name = 'cmsmasters_' . 'medical-clinic' . '_tweets_list_backup';

	$backup_tweets = get_option( $backup_name );

	if ( 'done' === get_transient( 'cmsmasters_' . 'medical-clinic' . '_tweets_list_regeneration' ) && ! empty( $backup_tweets ) ) {
		return $backup_tweets;
	}

	$cmsmasters_option = medical_clinic_get_global_options();

	if ( empty( $cmsmasters_option['medical-clinic' . '_twitter_access_token'] ) ) {
		return $backup_tweets;
	}
	...
	$response = wp_remote_get(
		...
		array(
			'headers' => array(
				'Authorization' => 'Bearer ' . esc_html( $cmsmasters_option['medical-clinic' . '_twitter_access_token'] ),
			),
			'body' => array(
	...
	set_transient( 'cmsmasters_' . 'medical-clinic' . '_tweets_list_regeneration', 'done', HOUR_IN_SECONDS * 6 );
...
/* Get Singular ID */
function medical_clinic_get_singular_id() {
	$id = false;
...
function medical_clinic_logo() {
	...
	if ($cmsmasters_option['medical-clinic' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['medical-clinic' . '_logo_title'] != '') {
			$blog_title = stripslashes($cmsmasters_option['medical-clinic' . '_logo_title']);
		} else {
		...
		if ($cmsmasters_option['medical-clinic' . '_logo_subtitle'] != '') {
			$blog_descr = stripslashes($cmsmasters_option['medical-clinic' . '_logo_subtitle']);
		} else {
...
function medical_clinic_get_comments($class = false, $show = false, $add_html = false) { 
	$out = '';
	
	if ($add_html) {
		$text = (get_comments_number() == '1') ? esc_html__('comment', 'medical-clinic') : esc_html__('comments', 'medical-clinic');
	} else {
		$text = '';
	}
...
/* Return content */
function medical_clinic_return_content($content) {
	return $content;
}
	...
	if( wp_verify_nonce($nonce, 'cmsmasters_ajax_like-nonce') ){
		...
		if ($post_ID != '') {
			...
			if (isset($_COOKIE['like-' . $post_ID]) || count($ipCheck) != 0) {
				...
				if ($add_html == 'true') {
					$text = ($plusLike == '1') ? esc_html__('like', 'medical-clinic') : esc_html__('likes', 'medical-clinic');
				} else {
					$text = '';
				}
...
function cmsmasters_ajax_apply_theme_style_callback() {
	...
	if (wp_verify_nonce($nonce, 'cmsmasters_ajax_apply_theme_style-nonce')) {
		if (isset($_POST['theme_style'])) {
			...
			update_option('cmsmasters_medical-clinic_theme_style_change', 'change');
		}
...
function medical_clinic_theme_style_change() {
	if (!CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		update_option('cmsmasters_medical-clinic_theme_style', '');

		return;
	}

	if ('hold' === get_option('cmsmasters_medical-clinic_theme_style_change')) {
		return;
	} else {
		...
		update_option('cmsmasters_medical-clinic_theme_style_change', 'hold');
	}
}
...
function cmsmasters_ajax_export_settings_callback() {
	...
	if (wp_verify_nonce($nonce, 'cmsmasters_ajax_export_settings-nonce')) {
		...
		echo medical_clinic_return_content($result);
