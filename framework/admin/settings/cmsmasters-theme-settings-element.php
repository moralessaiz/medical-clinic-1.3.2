...
function medical_clinic_options_element_fields($set_tab = false) {
	...
	case 'code':
		...
		$options[] = array( 
			'section' => 'code_section', 
			'id' => 'medical-clinic' . '_twitter_access_token', 
			...
			'desc' => sprintf(
				...
				esc_html__( 'Generate %s and paste Access Token to this field.', 'medical-clinic' ),
				'<a href="' . esc_url( 'https://api.cmsmasters.net/wp-json/cmsmasters-api/v1/twitter-request-token' ) . '" target="_blank">' .
					esc_html__( 'twitter access token', 'medical-clinic' ) .
				'</a>'
			), 
			'type' => 'text', 
			'std' => $defaults[$tab]['medical-clinic' . '_twitter_access_token'], 
			'class' => '' 
		);
