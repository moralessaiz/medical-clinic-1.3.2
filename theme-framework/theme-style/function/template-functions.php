	...
	if (!post_password_required()) {
		if (sizeof($images) > 1) {
			...
			'>';
				...
				foreach ($images as $image) {
					$image_atts = cmsmasters_check_img_agrs($image);
				...
				if ($cmsmasters_heading == 'custom') {
					...
					if (is_search()) {
						...
						if (!empty($wp_query->found_posts)) {
							echo sprintf(esc_html(_n('%1$d search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'medical-clinic')), $wp_query->found_posts, get_search_query());
						} else {
...
function medical_clinic_thumb_rollover($cmsmasters_id, $type = 'post-thumbnail', $rollover = true, $open_link = true, $group = false, $attachment_images = false, $attachment_video_type = false, $attachment_video_link = false, $attachment_video_links = false, $highImg = false, $show = true, $link_redirect = false, $link_url = false, $link_target = false, $icon = '', $placeholder_icon = 'cmsmasters_theme_icon_image', $puzzle_image = '') {
	$cmsmasters_option = medical_clinic_get_global_options();
